...
class MixNet(nn.Module):
    ...
    def forward(self, q_values, observations, hidden):
        ...
        print("q_values.unsqueeze(-1)", q_values.unsqueeze(-1))
...
def main(
    ...
    for episode_i in range(max_episodes):
        epsilon = max(
            min_epsilon,
            max_epsilon
            - (max_epsilon - min_epsilon) * (episode_i / (0.8 * max_episodes)),
        )
        state = env.reset()
        done = [False for _ in range(env.n_agents)]
        with torch.no_grad():
...
if __name__ == "__main__":
    ...
    parser.add_argument("--env-name", required=False, default="Fruits-v0")
    parser.add_argument("--seed", type=int, default=1, required=False)
    parser.add_argument("--no-recurrent", action="store_true", default=False)
    parser.add_argument("--max-episodes", type=int, default=25000, required=False)
    ...
    if USE_WANDB:
        ...
        run = wandb.init(project="qmix-gym", entity="lighthouse117", monitor_gym=True)
