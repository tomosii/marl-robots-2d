...
class Player:
    ...
    def rotate(self, keys, left, right):
        ...
        if keys[left]:
            self.rotation_angle += 1.5
        self.rotated_player = pygame.transform.rotate(
            self.player, (self.rotation_angle)
        )
...
class Bullet:
    def __init__(self):
        ...
        self.rotated_bullet = pygame.transform.rotate(
            self.bullet, player1.rotation_angle
        )
        self.time = 0
...
while run:
    ...
    window.fill(0)
    window.blit(
        player1.rotated_player, player1.rotated_player.get_rect(center=player1.pos)
    )
    for bullet in bullets:
        window.blit(
            bullet.rotated_bullet, bullet.rotated_bullet.get_rect(center=bullet.pos)
        )
    pygame.display.flip()
